	...
	return (<>
		...
		<Page>
			...
			<FormTable>
				...
				<CaptionCell>
					<OptionItemNameWithError error={errors.service_id}>
						{t("Наименование услуги") + ' *'}
					</OptionItemNameWithError>
				</CaptionCell>
				<ValueCell>
					<Select
						placeholder={t("Выбрать услугу...") + "..."}
						...
						noOptionsMessage={() => t("Введите название услуги")}
						styles={{
							input: base => ({
								...base, "input:focus": {
									boxShadow: "none"
					...
					<ul className='w-full'>
						...
						<CheckboxContainer>
							...
							<div className='w-44 md:ms-3 my-2 items-center flex'>
								...
								<div className='w-28 ms-4'>₪/{t("час")}</div>
							</div>
						</CheckboxContainer>
						...
						<CheckboxContainer>
							<OptionItemCheckboxField
								...
								label={t("Оплата по обьему работ")}
								onChange={(e) => {
			...
			<ActionsContainer className='items-end'>
				<CancelButton className='' label={t('Отмена')}
				              onClick={() => router.get(route('user.services'))}/>
				<ActionButton className='' label={t("Сохранить")}
				              onClick={submit}/>
			</ActionsContainer>
		</Page>
	</>);
